// Email Sorter AI - Sidebar Settings v3.6.1
...
function renderRulesList() {
  ...
  if (!container) return;
  if (rules.length === 0) { container.innerHTML = '<div class="empty-state">Zadna pravidla</div>'; return; }
  container.innerHTML = rules.map((rule, i) => '<div class="rule-item"><div class="rule-item-info"><h4>'+esc(rule.name||'Bez nazvu')+'</h4><p>'+esc(rule.type||'ostatni')+' \u2022 '+esc(rule.label||'')+' '+(rule.active?'\u2705':'\u274C')+'</p></div><div class="rule-item-actions"><button class="rule-edit-btn" data-i="'+i+'">Upravit</button><button class="rule-delete-btn" data-i="'+i+'">Smazat</button></div></div>').join('');
...
function testAPI(provider) {
  ...
  if (!key || key.trim().length === 0) { if (resultEl) { resultEl.textContent = 'Zadejte API klic'; resultEl.className = 'api-test-result error'; } return; }
  ...
  chrome.runtime.sendMessage({ action: 'testAPI', provider, apiKey: key, model: mod }, (response) => {
    ...
    if (chrome.runtime.lastError) { if (resultEl) { resultEl.textContent = chrome.runtime.lastError.message; resultEl.className = 'api-test-result error'; } return; }
    if (response && response.success) { if (resultEl) { resultEl.textContent = response.message; resultEl.className = 'api-test-result success'; } showToast('API funguje!', 'success'); }
    else { if (resultEl) { resultEl.textContent = response ? response.error : 'Chyba'; resultEl.className = 'api-test-result error'; } showToast('API test selhal', 'error'); }
  });
}
...
function editRule(index) {
  ...
  setVal('ruleName', rule.name || ''); setVal('ruleType', rule.type || 'ostatni'); setVal('ruleLabel', rule.label || '');
  ...
  setChecked('ruleStar', actions.includes('star')); setChecked('rulePriority', actions.includes('markImportant'));
  setChecked('ruleForwardEmail', actions.includes('forward')); setChecked('ruleCalendar', actions.includes('calendar'));
  setVal('ruleForwardTo', rule.forwardTo || ''); setVal('ruleCalendarTitle', rule.calendarTitle || ''); setVal('ruleCalendarDays', rule.calendarDays || 0);
  const fg = document.getElementById('forwardGroup'); const cg = document.getElementById('calendarGroup');
...
function saveRule() {
  const name = getVal('ruleName').trim(); const label = getVal('ruleLabel').trim();
  ...
  const rule = { id: editingRuleIndex >= 0 ? (currentSettings.rules[editingRuleIndex].id || name.toLowerCase().replace(/\s+/g,'-')) : name.toLowerCase().replace(/\s+/g,'-'),
  ...
  const ft = getVal('ruleForwardTo').trim(); if (ft) rule.forwardTo = ft;
  const ct = getVal('ruleCalendarTitle').trim(); if (ct) rule.calendarTitle = ct;
  rule.calendarDays = parseInt(getVal('ruleCalendarDays')) || 0;
  if (editingRuleIndex >= 0) { currentSettings.rules[editingRuleIndex] = rule; showToast('Pravidlo aktualizovano', 'success'); }
  else { if (!currentSettings.rules) currentSettings.rules = []; currentSettings.rules.push(rule); showToast('Pravidlo pridano', 'success'); }
  renderRulesList(); showModal(false);
}

function deleteRule(index) { if (confirm('Smazat pravidlo?')) { currentSettings.rules.splice(index, 1); showToast('Pravidlo smazano', 'success'); renderRulesList(); } }
...
function processEmails() {
  ...
  chrome.runtime.sendMessage({ action: 'processEmails', settings: currentSettings }, (response) => {
    ...
    if (response.success && response.data) {
      ...
      setText('overviewProcessed', response.data.processed || 0); setText('overviewRules', response.data.rulesMatched || 0); setText('overviewAI', response.data.aiMatched || 0);
    } else if (response.success === false) { showToast(response.error || 'Chyba', 'error'); }
  });
}
...
chrome.runtime.onMessage.addListener((req) => {
  if (req.action === 'statusUpdate') {
    chrome.runtime.sendMessage({ action: 'getStats' }, (s) => {
      ...
      setText('overviewProcessed', s.processed || 0); setText('overviewRules', s.rulesMatched || 0); setText('overviewAI', s.aiMatched || 0);
    });
  }
});
